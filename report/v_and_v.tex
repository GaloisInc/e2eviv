...
Testing serves the key functions of uncovering defects quickly, and
ensuring that previously-fixed defects do not recur in later
versions. The assurance testing provides comes from performing these
functions. It is impossible for testing to reveal all possible defects
...
of some defects provide more assurance than an untested system.
...
defects that are beyond the scope or capability of the formal
techniques.
...
particular values that would expose a defect, and so the test suite
will succeed despite the presence of that defect.

Developer intuition and understanding of the implementation increases
the likelihood that unit tests will exercise code containing a defect,
but tests still cannot be exhaustive. Code coverage, the percentage of
code exercised by a given test suite, is often used to make unit tests
more comprehensive, but in practice high coverage has not been shown
to necessarily uncover more defects~\cite{inozemtseva2014coverage}.
