...
\todo{I'm not sure the following para is relevant in the presence of
  the bulletin board, which contains all the information needed for
  election officials to compute election outcome. In particular,
  attempts to manipulate the election will be detected based on the
  use of E2E protocols, and not based on physical security. Poorvi}
\todo{I agree to a point: the kind of trust engendered in these kinds
  of access controls are more practical and psychological than
  theoretical. They help, but are not strictly necessary.  I'm going
  to leave the paragraph in for the moment if only because so many
  electoral authorties have, and will continue to want, such
  controls. -JRK} 
...
E2E-VIV systems are designed to be software independent.
...
\todo{Notice that the second sentence is inconsistent with the
  coercion-resistance requirement we have been discussing over email I
  added the last sentence. Poorvi} \todokiniry{While true, this does
  not invalidate the introduction of the relevant mandatory or
  recommended requirement. -JRK} 
...
of such monitoring can be improved through standard secure computing
practices such as malware scanning and the use of reverse firewalls
(preventing the computer from making outgoing connections that are not
approved by the user or by system security policies). However, we can
only recommend that voters take such protective measures, and the
voting client software cannot detect whether they are actually in
place. Additionally, it is possible that a rare effort towards such
monitoring is not detected by malware scanning or the use of reverse
firewalls.
