\chapter{Architecture\ifdraft{ (Joe K./Dan) (95\%)}{}}
...
cryptographic foundations of E2E-VIV protocols have been developed to a
...
However, E2E-VIV protocols do not prevent such errors, and an implemented E2E-VIV system should minimize the chances of there being such errors. For this reason, there are many open engineering issues associated
with actually building, running, and maintaining an E2E-VIV system that
...
scenarios---is vital to actually implementing a successful E2E-VIV
system. \todo{I made changes to the early part of the above para to clarify that E2E protocols only detect problems but do not prevent hem. This we must do everything else necessary for the "security" and reliability of the outcome--Poorvi}
...
\todokiniry{I'm not sure what exactly you're going for with respect to
  NIST component-based certification. -dmz}
...
\todo{I changed the first couple of paras which seem to be oriented towards E2E systems which provide computational integrity and unconditional privacy. The practical E2E systems, though, or at least the ones that have been used in elections (PaV, Helios, Scantegrity) all provide unconditional integrity and computational privacy. I've changed the description to reflect this. If there was a particular voting system that you had in mind that is not covered by my description, perhaps one can add another para? Note that I use the following sentence that privacy advocates might jump upon: "This also means that fewer than 7 officials cannot compromise vote secrecy." Poorvi}
% The public must be able
...
\todo{I'm not sure the following para is relevant in the presence of the bulletin board, which contains all the information needed for election officials to compute election outcome. In particular, attempts to manipulate the election will be detected based on the use of E2E protocols, and not based on physical security. Poorvi} Another mechanism to increase trust in the electoral authority is
...
also corrupt its self-certification mechanism. Similarly, a voter relies on the client to check the signature whether using code signing or self-certification, and a malicious client can modify the software and pretend it passed the signature check. \todo{Added this last sentence: Poorvi} 
...
E2E-VIV systems are designed to be software independent. \todo{Added previous sentence and modified the one before it: Poorvi}
...
\todo{Notice that the second sentence is inconsistent with the coercion-resistance requirement we have been discussing over email I added the last sentence. Poorvi}
...
cannot address as part of an E2E-VIV system implementation, is
...
malicious action on the part of untrusted client systems. A good E2E-VIV system will enable a voter to detect any such effort that changes election outcome. Additionally, an E2E-VIV system with dispute resolution will enable a voter to prove that there is a problem. 
% Essentially,
...
\todo{The sentence I commented out is not true. I added two sentences: Poorvi}
...
The use of a public append-only bulletin board with vote encryptions published in a tamper-evident and encouraging voters and observers to frequently check the board provides a means of detecting data manipulation, while the above safeguards make it additionally difficult. \todo{added the last para because we need to communicate that E2E goes beyond the standard security safeguards: Poorvi}
...
\todo{The following para contradicts the earlier requirement of coercion-resistance we are discussing over email: Poorvi}
Of course, it is impossible to defend an E2E-VIV system against
...
\item \textbf{Key Distribution Method} \ In all the possible
  ...
  continually generated and exchanged throughout the system. \todo{Modified the second to last sentence: Poorvi}
