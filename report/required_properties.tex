...
\todo{``This must be the case even
...
completed ballot.'' I believe this is misleading. I do not know of such an E2E protocol. In order to work, it must allow the voter to vote multiple times. Yet, it must allow it to appear that the video-recorded vote was cast, hence its encryption must be among those counted in a verifiable manner. Yet, this vote must not count, and the later vote should replace it. I can think of a possible approach to solve this problem, but I do not know of any protocol that has this functionality now, and I know of at least one effort to develop such a protocol that has hit road blocks. -Poorvi}
...
notice) from the system, the ballot casting process is complete and the system has recorded the vote. \todo{I did not want to use ``she can be certain that her ballot was recorded correctly'' because that gives a verifiablility flavor to the statement but verifying a vote is not part of the casting process. -- Poorvi}  
%she can be certain that her ballot was
% recorded correctly. 
...
these should be identical, \todo{``except that details about software used
solely to protect the system against attacks may be omitted from the
public manifest for security reasons'' -- I don't understand how this is not ``security by obscurity''? Also, how does one know what the purpose of the secret software is if it cannot be examined? I don't think anything about the software should be secret. Passwords and keys may be kept secret -- Poorvi}. Well-defined procedures must
...
it to any other person or to remove it from the polling station. \todo{Note that the only protocols with receipt freedom are those that assume a private channel with the voting system and/or a trusted voting computer. This is not a reasonable assumption in a remote voting scenario, and you might want to remove this para or emphasize that it describes a requirement of polling booth systems. -- Poorvi} 
