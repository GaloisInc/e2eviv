...
notations are easy to work with and manipulate. Current UML supports
only a graphical notation, though there is an unsupported official
``Human-Usable Textual Notation'' for UML~\cite{HUTN}; it was last
updated in 2004, reflects only the version of UML that was current at
the time, and is not as expressive as graphical UML. Currently, there
...
as graphical UML and most of which have significant readability
issues.
...
\emph{reversibility}. Seamlessness is the property that allows BON
models to be smoothly (and, in many cases, completely automatically)
translated to lower-level specification languages, and further to
executable implementations. Reversibility is the property that allows
consistency to be maintained between the models, which serve as an
important part of the documentation of the resulting system, and the
resulting implementations---when the implementation is changed, that
change can be (again, often completely automatically) propagated back
up to the BON model.
...
process. For example, the BON representation of the E2E VIV
requirements in \autoref{appendix:bon_requirements} consists almost
entirely of requirements expressed using simple English sentences; it
is therefore far more accessible to a wide audience than an equivalent
set of UML diagrams would be.

Finally, BON is \emph{small}. Its specification is a fraction of the
size of the UML specification, and its visual representation is
significantly less complex; for example, it has only two types of
arrowhead for connections among entities, in contrast to UML's seven
types plus additional adornments. \tododmz{I think this paragraph, if
  we keep it, could be much better.}
