...
does not specify \emph{how} choices in contests are
selected. Constraints such as these on the realization and behavior of
the system are provided in general terms by the requirements,
scenarios, and informal concept specifications, and are precisely
...
should preferably be done automatically as part of automated testing
and continuous integration processes, in a way that makes it clear to
...
\todo{I'm not sure we actually need subsections on why these various
  aspects of specification are useful, given that there's a bit of
  that in all the other subsections already... -Dan}
...
these specifications, such as the architecture specification, may be
minimal for simple systems. We describe each of these in turn.
...
Features may be private, accessible only to a given concept or group
of concepts, or may be public, accessible to the entire system. It is
possible for a concept to have private features that are not direct
refinements of its informal queries and commands. For example, the
``election'' concept might have restricted features representing the
entire database of candidates for office and ways to update that
database, but only allow unrestricted access to the database via a
query requesting the candidates for a particular race.
...
postconditions and invariants are a part of the Design by Contract
development technique~\cite{OOSC}: a precondition is a statement
that must be true in order to invoke a feature; a postcondition is a
statement that must be true after a feature has been invoked; and an
invariant is a statement that must be true at all (observable)
...
in precise mathematical terms that can be checked for logical
consistency and various other desirable properties before
implementation begins; this allows for early detection of problems
with the requirements or their realizations. Second, it can be used
with automated code generation tools to create a significant amount of
the implementation and source code specification automatically, in a
provably-correct and traceable fashion.
...
statements about its behavior. Source code specifications are
...
implementation language has a significant effect on the types of
source code specifications that can be written and the effort it takes
to write them. With appropriate tool support, which is available for
several implementation languages of various styles, many source code
specifications can be generated automatically from formal concept
specifications, and some can be inferred from parts of the
implementation.

The main purpose of source code specifications is to enable automated
tools to verify or validate the associated implementations. Static
analyzers can verify, using automated theorem proving techniques, that
implementations satisfy their formal specifications without the need
to actually run the code. Runtime assertion checking compilers can use
the specifications to generate executable code that contains
continuous runtime checks, so that any runtime violation of the
specification will be detected. Automated test generators can use the
specifications to generate high-coverage unit test suites to exercise
the finished implementation. All of these can significantly increase
the reliability of the software, and in most cases they require
minimal developer effort after the specifications have been written. 
